...
const Store = (() => {
  ...
  const defaultState = () => ({
    ...
    checklists: {},  // "A01:0" -> true when ticked
    notes: '',
    lang: 'en',
    certName: '',
    termOpen: false,
  ...
  function checklistKey(code, idx) { return code + ':' + idx; }

  function isChecklistDone(code, idx) {
    return !!(state.checklists && state.checklists[checklistKey(code, idx)]);
  }

  function toggleChecklist(code, idx) {
    if (!state.checklists) state.checklists = {};
    const k = checklistKey(code, idx);
    if (state.checklists[k]) delete state.checklists[k];
    else state.checklists[k] = true;
    save();
  }

  function checklistProgress(code, total) {
    let done = 0;
    for (let i = 0; i < total; i++) if (isChecklistDone(code, i)) done++;
    return { done, total };
  }
  ...
  return {
    ...
    isChecklistDone, toggleChecklist, checklistProgress,
    reset, exportJSON,
  };
})();
