...
app.get('/api/orders/:id', requireAuth, (req, res) => {
  ...
  },
  {
    ...
    blurb: M('Слабые хэши, MD5, JWT alg:none, plaintext secrets.', 'Weak hashes, MD5, JWT alg:none, plaintext secrets.'),
    theory: M(
...
function verify(token) {
  ...
  },
  {
    ...
    flag: 'FLAG{sqli_union_leaks_secrets}',
    blurb: M('SQLi, XSS, OS command injection — данные становятся кодом.', 'SQLi, XSS, OS command injection — data becomes code.'),
    theory: M(
...
const row = db.prepare(
  ...
  },
  {
    ...
    blurb: M('4-digit OTP без rate-limit, security questions, business logic flaws.', '4-digit OTP without rate-limit, security questions, business logic flaws.'),
    theory: M(
...
if (!timingSafeEqual(otp, user.otp)) {
  ...
  },
  {
    ...
    blurb: M('DEBUG=True, default passwords, open S3, verbose errors.', 'DEBUG=True, default passwords, open S3, verbose errors.'),
    theory: M(
...
app.use((err, req, res, next) => {
  ...
  res.status(500).json({ error: 'Internal error' });
    ...
    quiz: [
      ...
      },
      {
        ...
        opts: [
          M('Открытый Kubernetes Dashboard без аутентификации', 'An open Kubernetes Dashboard without authentication'),
  ...
  },
  {
    ...
    blurb: M('Outdated deps, Log4Shell, supply chain, missing SBOM.', 'Outdated deps, Log4Shell, supply chain, missing SBOM.'),
    theory: M(
...
// Dependabot + SLSA provenance`,
    quiz: [
      ...
      {
        q: M('Атака SolarWinds (2019) — классический пример:', 'The SolarWinds attack (2019) is a classic example of:'),
        opts: [
        ...
        ans: 0,
        e: M('SolarWinds (2019) — классическая компрометация доверенного вендора: вредоносный код распространялся через легитимное обновление Orion, затронув тысячи организаций.', 'SolarWinds (2019) is the classic trusted-vendor compromise: malware shipped through a legitimate Orion update, affecting thousands of organizations.'),
      },
      {
  ...
  },
  {
    ...
    blurb: M('Credential stuffing, weak sessions, no MFA, predictable cookies.', 'Credential stuffing, weak sessions, no MFA, predictable cookies.'),
    theory: M(
...
res.cookie('session', sid, { httpOnly: true, secure: true, sameSite: 'lax' });`,
  ...
  },
  {
    ...
    blurb: M('Insecure deserialization, unsigned updates, CI/CD poison.', 'Insecure deserialization, unsigned updates, CI/CD poison.'),
    theory: M(
...
// role ALWAYS from DB, never from client`,
  ...
  },
  {
    ...
    blurb: M('No audit trail, 207 days MTTD, secrets in logs.', 'No audit trail, long MTTD, secrets in logs.'),
    theory: M(
...
const CVES = [
  ...
  {id:'CVE-2024-1708',y:2024,d:'ConnectWise ScreenConnect path traversal auth bypass.',v:8.4,s:'HIGH',c:'CWE-22',o:'A01',ve:'ConnectWise',p:'ConnectWise'},
