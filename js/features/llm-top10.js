...
<li><strong>OWASP LLM Top 10 (2025):</strong> <em>LLM09 Misinformation</em>; IP/training-data lookalikes рядом с <em>LLM03 Supply Chain</em> / governance коммерческого использования output.</li>
  ...
  function renderHub() {
    ...
    return `
      <div class="lab-page">
        ${Owasp.tabs('llm')}
        <div data-watermark="LLM" class="hero hub-compact">
        ...
        <div class="card">
          <div class="cb theory hub-intro-full" id="llm-intro">${en ? INTRO.en : INTRO.ru}</div>
        </div>

        <div class="llm-area-list">
          ${ITEMS.map(it => `
            <details class="card llm-area" id="llm-${esc(it.id)}" name="llm-application-areas">
              <summary class="ch llm-area-head">
                <span class="lab-code">${esc(numLabel(it.id))}</span>
                <span class="llm-area-title">${esc(en ? it.title_en : it.title_ru)}</span>
                <span class="llm-area-chevron" aria-hidden="true">›</span>
              </summary>
              <div class="cb theory llm-area-body">
        ...
        <div class="card" style="color:var(--txt)">
          ...
          <div class="cb row" style="flex-wrap:wrap;gap:8px">
            ...
            <button class="btn" onclick="UI.route('webtop10')">Web Top 10</button>
  ...
  function openArea(id) {
    const box = document.getElementById('llm-' + String(id || ''));
    if (!box) return;
    box.open = true;
  ...
  return { renderHub, openArea, ans, ITEMS };
})();
