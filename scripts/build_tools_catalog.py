...
def offensive_phase(item: dict) -> str | None:
    ...
    if track not in OFFENSIVE_PHASES:
        return None
    return OFFENSIVE_EXCEPTIONS.get(
        item["id"],
        OFFENSIVE_PHASES[track].get(item["category"], "vulnerability-discovery"),
    )
...
def render() -> str:
    ...
    return (
        "/* Generated by scripts/build_tools_catalog.py from synchronized Markdown sources. */\nconst TOOLS_CATALOG = "
...
def main() -> None:
    ...
    if args.check:
        if CATALOG.read_text(encoding="utf-8") != output:
            raise SystemExit(
                "js/tools/catalog.js is out of date; run scripts/build_tools_catalog.py"
            )
        return
